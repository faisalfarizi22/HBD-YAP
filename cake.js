document.addEventListener("DOMContentLoaded", function() {
    ...
    // Detect sound and blow out candles
    navigator.mediaDevices.getUserMedia({ audio: true })
        .then(stream => {
            ...
            scriptProcessor.onaudioprocess = function() {
                ...
                if (volume > 50) { // Adjust this threshold as necessary
                    document.querySelectorAll('.flame').forEach(flame => flame.classList.add('hide'));
                    document.querySelector('.light-candle-icon').style.display = 'block';
                    playCelebrationEffects();
                }
            };
        })
        .catch(err => console.error('Microphone not accessible: ', err));
    ...
    window.lightCandles = function() {
        document.querySelectorAll('.flame').forEach(flame => flame.classList.remove('hide'));
        document.querySelector('.light-candle-icon').style.display = 'none';
    }
});
